...
export default function HomePage() {
  ...
  // ── Automatic Room Creation (Immediate Editor for Guests) ──────────────────
  useEffect(() => {
    // Only auto-create if we are on the home page and not already creating/loading
    if (!loading && !creating && location.pathname === '/') {
      // If we don't have a username, use a default one
      const nameToUse = username || 'GuestUser';
      if (!username) setUsername(nameToUse);
      
      // Small delay to ensure state is ready
      const timer = setTimeout(() => {
        triggerAutoCreate(nameToUse);
      }, 100);
      return () => clearTimeout(timer);
    }
  }, [loading]);

  async function triggerAutoCreate(nameToUse) {
    if (!nameToUse) return;
    ...
    try {
      ...
      if (data.roomId) {
        sessionStorage.setItem('cs_username', nameToUse)
        if (data.ownerToken) localStorage.setItem(`cs_owner_${data.roomId}`, data.ownerToken)
